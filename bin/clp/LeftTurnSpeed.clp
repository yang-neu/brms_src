;==================================================
; Function(LeftTurnSpeed.clp)
;==================================================
;最大舵角を算出する
(deffunction SpecificAgenda::calcLeftSteeringAngleMax (?acce ?calc)
	...
	(return ?calc))
;アクセル運転状態判定
(deffunction SpecificAgenda::judLeftTurnState(?calc ?data)
...
;最頻度の特性に更新する
(deffunction SpecificAgenda::calcLeftTurnProperty(?data)
	...
	(return ?data))
;==================================================
; Rule(LeftTurnSpeed.clp)
;==================================================
(defrule SpecificAgenda::車速20㎞/h以上時：左カーブのstep開始状態検出
	(declare (salience 600))
	
	?acce <- (EventCharacteristicAndStatusBinomial (from entryPoint) (name "Characteristic And Status Stream") (type LEFR_TURN_SPEED) (step ?step&:(eq ?step STEP_END)))
	...
	(printout t "⑱車速20㎞/h以上時：左カーブのstep開始状態検出:step:" (fact-slot-value ?acce step) crlf))

(defrule SpecificAgenda::左カーブのシーン成立時：左カーブ開始時刻検出
	(declare (salience 600))
	
	?acce <- (EventCharacteristicAndStatusBinomial (from entryPoint) (name "Characteristic And Status Stream") (type LEFR_TURN_SPEED) (step ?step&:(eq ?step STEP_START)))
	(EventDriveScene (from entryPoint) (name "Current Scene Stream") (driveScene ?driveScene))
	(EventVehiclePostureScene (from entryPoint) (name "Current Scene Stream") (posture TURN_LEFT) (beforeVehiclePostureScene TURN_STRAIGHT))
	=>
...
(defrule SpecificAgenda::左カーブのシーン成立中の最大舵角を算出する
	(declare (salience 600))
	
	?calc <- (EventCharacteristicAndStatusBinomial (from entryPoint) (name "Characteristic And Status Stream") (type LEFR_TURN_SPEED) (step ?step&:(eq ?step STEP_CALC)))
	?acce <- (EventSteeringAngleList (from entryPoint) (name "Receiving Data Stream"))
	=>
	(bind ?calc (calcLeftSteeringAngleMax ?acce ?calc))
	(printout t "⑱左カーブのシーン成立中の最大舵角を算出する : " (fact-slot-value ?calc steeringAngleMax) "steeringAngleMax" crlf))

(defrule SpecificAgenda::計算:左カーブの最大舵角が150度以上　かつ、シーン成立時間が2s以上の時
	...
	(bind ?calc (judLeftTurnState ?calc ?driveScene))
	(bind ?calc (calcLeftTurnProperty ?calc))
	(bind ?calc (modify ?calc (step STEP_END)))
	(duplicate ?calc (name "Calculation Stream")))

(defrule SpecificAgenda::左カーブのシーンを抜けた時:step終わり状態検出
	(declare (salience 400))
	
	?calc <- (EventCharacteristicAndStatusBinomial (from entryPoint) (name "Characteristic And Status Stream") (type LEFR_TURN_SPEED) (step ?step&:(neq ?step STEP_END)))
	(or (EventSpeed (from entryPoint) (name "Current Receiving Data Stream") (speed ?speed&: (< ?speed 20)))
		(EventVehiclePostureScene (from entryPoint) (name "Current Scene Stream") (posture ?posture&:(neq ?posture TURN_LEFT)&:(eq ?step STEP_CALC))))
	=>
	(bind ?calc (modify ?calc (step STEP_END)))
	(printout t "⑱車速20㎞/h未滿時：step終わり状態検出:step:" ?step crlf))

(defrule SpecificAgenda::集計：左カーブスピード：サンプル未達
	(declare (salience 200))

	?margin <- (EventCharacteristicAndStatusBinomial(from entryPoint) (name "Calculation Stream") (type LEFR_TURN_SPEED) (specificList $?specificList&:(< (length$ ?specificList) 100)))
	=>
...
(defrule SpecificAgenda::集計：左カーブスピード：特性未定義
	...
	?calc <- (EventCharacteristicAndStatusBinomial (from entryPoint) (name "Calculation Stream") (type LEFR_TURN_SPEED) (specificList $?specificList&:(>= (length$ ?specificList) 100)) (specific NAN))
	=>
...
(defrule SpecificAgenda::集計：左カーブスピード：スローインファストアウトカーブ
	...
	?calc <- (EventCharacteristicAndStatusBinomial (from entryPoint) (name "Calculation Stream") (type LEFR_TURN_SPEED) (specificList $?specificList&:(>= (length$ ?specificList) 100)) (specific SLOW_IN_FAST_OUT))
	=>
...
(defrule SpecificAgenda::集計：左カーブスピード：減速カーブ
	...
	?calc <- (EventCharacteristicAndStatusBinomial (from entryPoint) (name "Calculation Stream") (type LEFR_TURN_SPEED) (specificList $?specificList&:(>= (length$ ?specificList) 100)) (specific REDUCE_TURN))
	=>
...
(defrule SpecificAgenda::集計：左カーブスピード：非減速カーブ
	...
	?calc <- (EventCharacteristicAndStatusBinomial (from entryPoint) (name "Calculation Stream") (type LEFR_TURN_SPEED) (specificList $?specificList&:(>= (length$ ?specificList) 100)) (specific ACCEL_TURN))
	=>
