...
static struct defrule *CreateNewDisjunct(
  ...
  struct joinNode *lastJoin)
  {
   ...
   //madianyi Begin
   newDisjunct->localFactList = NULL;
   newDisjunct->lastLocalFact = NULL;
   //madianyi End
   ...
   if (list->type == FCALL)
     {
      ...
      //madianyi Begin
      else if (list->value == (void *) FindFunction(theEnv,"set"))
        {
         if (UpdateModifyDuplicate(theEnv,list,"set",VtheLHS) == FALSE)
           return(-1);
        }
      //madianyi End
      return(0);
