...
/***************************************/
   ...
   static void                    DuplicateModifyCommand(void *,int, int, DATA_OBJECT_PTR);
   //madianyi End
...
globle void DeftemplateFunctions(
  void *theEnv)
  {
   ...
   //madianyi Begin
   EnvDefineFunction(theEnv,"set",'u', PTIEF SetCommand,"SetCommand");
   //madianyi End
...
globle void ModifyCommand(
  ...
  DATA_OBJECT_PTR returnValue)
  {
   ...
   DuplicateModifyCommand(theEnv,TRUE,TRUE,returnValue); //A new argument(assertIt) added for DuplicateModifyCommand
   //madianyi End
  }
...
globle void DuplicateCommand(
  ...
  DATA_OBJECT_PTR returnValue)
  {
   ...
   DuplicateModifyCommand(theEnv,FALSE,TRUE,returnValue); //A new argument(assertIt) added for DuplicateModifyCommand
   //madianyi End   
  }

//madianyi Begin
/***************************************************************************/
/* SetCommand: H/L access routine for the set command. Calls   */
/*   the DuplicateModifyCommand function to perform the actual work.       */
/***************************************************************************/
globle void SetCommand(
  void *theEnv,
  DATA_OBJECT_PTR returnValue)
  {
   DuplicateModifyCommand(theEnv,FALSE,FALSE,returnValue);
  }
//madianyi End
...
static void DuplicateModifyCommand(
  ...
  DATA_OBJECT_PTR returnValue)
  {
   ...
   if (computeResult.type == INTEGER)
     {
      ...
      if (factNum < 0)
        {
         //madianyi Begin
         if (!assertIt) ExpectedTypeError2(theEnv,"set",1);
         //if (retractIt) ExpectedTypeError2(theEnv,"modify",1);
	  else if (retractIt) ExpectedTypeError2(theEnv,"modify",1);
	  //madianyi End
   ...
   //madianyi Begin
   /*==================================*/
   /* See if oldFact has a Local Fact. If so, replace it with  */
   /* the Local Fact.
   /*==================================*/

   if (oldFact->localFact != NULL) //oldFact has been "set" before, so it has a Local Fact.
     {
      tempFact = oldFact; 
      oldFact = oldFact->localFact;
     }
   //madianyi End
   ...
   //madianyi Begin
   if (assertIt == FALSE && tempFact != NULL) //oldFact has been "set" before, and this time "set" again
     {
      newFact = tempFact->localFact;
     }
   //madianyi End
   //madianyi Begin
   else {
   ...
   //madianyi Begin
   if (oldFact != newFact) //when oldFact == newFact == localFact, it means set command for oldFact, and also it's not the first time.
   	{
   //madianyi End
   for (i = 0; i < (int) oldFact->theProposition.multifieldLength; i++)
     {
   ...
   if (tempFact != NULL) oldFact = tempFact;
   //madianyi End
   ...
   //madianyi Begin
   //"set" command first time for oldFact
   else if (oldFact->localFact == NULL) 
     {
      /*========================================*/
      /* Link newFact to localFact of oldFact */
      /*========================================*/
      oldFact->localFact = newFact;
      /*===============================================*/
      /* Add oldFact to localFactList of ExecutingRule */
      /*===============================================*/
      ExecutingRule = EngineData(theEnv)->ExecutingRule;

      oldFact->nextLocalFact = NULL;
      oldFact->previousLocalFact = ExecutingRule->lastLocalFact;
      if (ExecutingRule->lastLocalFact == NULL)
      	 { ExecutingRule->localFactList = oldFact; }
      else
      	 { ExecutingRule->lastLocalFact->nextLocalFact = oldFact; }
      ExecutingRule->lastLocalFact = oldFact;
      /*========================================*/
      /* The oldFact will be return value */
      /*========================================*/
      theFact = oldFact;
     }
   else { theFact = oldFact; }
   //madianyi End
   /*========================================*/
...
//madianyi Begin
/*************************************************/
/* DuplicateParse: Parses the set command. */
/*************************************************/
globle struct expr *SetParse(
  void *theEnv,
  struct expr *top,
  const char *logicalName)
  {
   return(ModAndDupParse(theEnv,top,logicalName,"set"));
  }
//madianyi End
